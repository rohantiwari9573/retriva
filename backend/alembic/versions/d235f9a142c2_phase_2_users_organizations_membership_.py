"""phase 2: users, organizations, membership, refresh tokens

Revision ID: d235f9a142c2
Revises: 
Create Date: 2026-09-12 12:46:38.585220
"""
...
def downgrade() -> None:
    ...
    # Alembic's autogenerate does not track the lifecycle of the Postgres
    # ENUM type it creates for organization_members.role - dropping the table
    # does not drop the type, so a manual DROP TYPE is required here or a
    # rollback leaves an orphaned "org_role" type that collides with the next
    # upgrade's CREATE TYPE.
